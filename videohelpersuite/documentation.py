...
descriptions = {
  ...
  "VHS_DiskColorMatch": ['Disk Color Match 🎥🅥🅗🅢', short_desc('Shot-match a disk clip with one locked color transform, so moving objects do not pump the grade'),
    'Stats are gathered from sampled frames, then the same Reinhard/MKL grade is applied to every frame. Not a per-frame ColorMatchV2.',
    {'Inputs': {
        'disk': 'Clip to grade',
        'image_ref': 'Still that defines the look',
        'disk_ref': 'Other clip whose overall look to match (not frame-aligned)',
        },
     'Widgets': {
         'method': 'reinhard_lab is the usual video shot match. mkl_rgb uses a 3x3 color matrix',
         'source_stats': 'Where source mean/std come from. sampled (default) is robust to a weird first frame',
         'chroma_only': 'Keep source luma, shift color only — best when objects move through the shot',
